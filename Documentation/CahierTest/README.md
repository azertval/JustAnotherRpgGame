...
**959 cas de test**, un par test automatisé du dépôt. Le cahier est **engendré** depuis les blocs `\castest{…}` écrits au-dessus de chaque test par `scripts/generate_cahier_test.py` : il ne s'édite pas — on corrige le commentaire du test, puis on relance le script. La CI refuse un cahier périmé, et refuse un test sans bloc.
...
| [HMI · Graphics](hmi-graphics.md) | Tests unitaires | 184 | 34 | 59 | 83 | 8 |
